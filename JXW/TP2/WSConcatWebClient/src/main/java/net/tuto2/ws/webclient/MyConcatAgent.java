 ...
 * 
 */
@WebServiceClient(name = "MyConcatAgent", targetNamespace = "http://concat.service.ws.tuto2.net/", wsdlLocation = "http://localhost:8080/services/concat?wsdl")
public class MyConcatAgent
    extends Service
{

    private final static URL MYCONCATAGENT_WSDL_LOCATION;
    private final static Logger logger = Logger.getLogger(net.tuto2.ws.webclient.MyConcatAgent.class.getName());

    static {
        URL url = null;
        try {
            URL baseUrl;
            baseUrl = net.tuto2.ws.webclient.MyConcatAgent.class.getResource(".");
            url = new URL(baseUrl, "http://localhost:8080/services/concat?wsdl");
        } catch (MalformedURLException e) {
            logger.warning("Failed to create URL for the wsdl Location: 'http://localhost:8080/services/concat?wsdl', retrying as a local file");
